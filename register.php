...
$username_err = $password_err = $confirm_password_err = $email = $confirm_email_err ="";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($username_err) && empty($password_err) && empty($confirm_password_err) && empty($email_err) && empty($confirm_email_err)){
        ...
        $sql = "INSERT INTO users (username, password, email, subscriber) VALUES (?, ?, ?, ?)";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssss", $param_username, $param_password, $param_email, $subscriber);
...
<body>
    <div class="wrapper">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
			...
			<font size="-5">By clicking Submit, you are agreeing to our <a href="https://retrobit.games/terms.html" target="_blank">Terms of Service</a> and Privacy Policy. If you do not agree, you cannot use RetroBit.Games</font><br><br><br>
